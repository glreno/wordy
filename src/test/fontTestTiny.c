...
int main(void)
{
    ...
    vfm_initialize(&fontmanager0,&AQORDLFONTDEF,1,FONTLIST0);
    ...
    // Initialize six vw_tiny views
    // Yellow and Grey are both 5x5, so are a letter short.
    // So there is also a very tiny one-row vw_tiny for z
    // And also three for Green, since each fontmanager can only handle 10 green characters.
    vw_initialize(&fontmanager0,5,2,1,&grey);
    vwt_initialize(&grey);
    vw_initialize(&fontmanager0,5,2,22,&yellow);
    vwt_initialize(&yellow);
    vw_initialize(&fontmanager0,1,13,1,&green1);
    vwt_initialize(&green1);
    vw_initialize(&fontmanager1,2,16,1,&green2);
    vwt_initialize(&green2);
    vw_initialize(&fontmanager2,2,21,1,&green3);
    vwt_initialize(&green3);
